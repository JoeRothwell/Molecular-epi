...
## The Lifepath breast cancer case-control study
...
Baseline characteristics of participants are as follows.
...
### Some background on NMR metabolomics
...
library(kableExtra)
knitr::kable(ints[1:5, 1:10], format = "html") %>% kable_styling(bootstrap_options = c("condensed"))
```
...
```{r totalint, message = F, warning  = F, fig.width=8, fig.height=3}
...
### Sources of variability in metabolomics data
...
```{r pcpr2, echo=F, warning=F, message=F, results=F, cache=T}
...
```{r clr2-model, echo = F, message = F, warning = F, fig.width=8, fig.height=10, cache=T}
ints <- read_tsv("1507_XMetabolite_std_cpmg_E3N.txt")
