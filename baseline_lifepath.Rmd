...
Baseline characteristics for Lifepath

```{r, results = "asis", echo = F, message = F, warning = F}
library(tidyverse)
meta <- read.csv("Lifepath_meta.csv", na.strings = "9999")
meta0 <- meta %>% 
 ...
 lifepath_summ <-
   list("Total subjects"   =
        ...
        "Age at blood collection (years)" = 
          list("Mean"     =  ~ mean_sd(AGE)),
        "BMI" =
          list("Mean (SD)" = ~ mean_sd(BMI)),
        "Waist circumference" =
          list("Mean (SD)" = ~ mean_sd(HANCHE)),
        "Menopausal status at blood collection" =
          list("Pre-menopausal"    =  ~ n_perc0(MENOPAUSE == 0),
               "Post-menopausal"   =  ~ n_perc0(MENOPAUSE == 1)),
        "Smoking status" =
          list("Yes" =  ~ n_perc0(SMK == 1),
               "No"   =  ~ n_perc0(SMK == 0)),
        "Diabetic status" =
          list("No" =  ~ n_perc0(DIABETE == 0),
...
st <- summary_table(group_by(meta0, CT), lifepath_summ)
