...
# Read in CRC case control dataset (2) and subset the 146 biocrates compounds
library(haven)
wcrf <- read_dta("Wcrf_Score.dta") %>% select(Idepic, Wcrf_C_Cal)

crc2 <- read_csv("data/biocrates_p150.csv")
crc2 <- crc2 %>% left_join(wcrf, by = "Idepic")

set3 <- crc2 %>% select(Acylcarn_C0 : Sugars_H1, -Acylcarn_C14, -Acylcarn_C16_1_Oh, -starts_with("Outdq_"))
set3 <- set3 %>% select(sort(current_vars()))

data_frame(colnames(set2), colnames(set3))
